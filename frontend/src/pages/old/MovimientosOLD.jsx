...
function getDestinoOptions(origenTipo) {
  ...
  if (origenTipo === "Osana") return ["Vivero"];
...
function getFormErrors(form) {
  ...
  if (
    ["Osana", "Otro", "Palmetum", "Empresa", "Organismo oficial", "Colegio"].includes(form.origen_tipo) &&
    ...
    return safeArray(pedidos)
      ...
      .filter((p) => {
        ...
        const base = [
          ...
          p?.barrio_destino,
          p?.direccion_destino,
          ...(safeArray(p?.items).map((it) => `${it?.producto_nombre_cientifico || it?.nombre_cientifico || it?.producto_nombre || ""} ${it?.tamano || ""} ${it?.cantidad || ""}`)),
        ]
          .filter(Boolean)
          .join(" ")
          .toLowerCase();
                    ...
                    {safeArray(p.items).map((it, idx) => (
                      ...
                      >
                        <div style={{ fontWeight: 800, color: "#0f172a" }}>
                          {it.producto_nombre_cientifico || it.nombre_cientifico || it.producto_nombre || `Producto #${it.producto_id}`}
                        </div>
...
function MovimientoModal({
  ...
  const productosDisponiblesParaOrigen = useMemo(() => {
    if (form.origen_tipo !== "Vivero") return safeArray(productos);

    return safeArray(productos)
      .filter((p) =>
        TAMANOS.some((tamano) =>
          ZONAS.some((zona) => {
            const key = buildStockKey(p.id, zona, tamano);
  ...
  }, [form.origen_tipo, productos, stockByProductZoneSize]);

  useEffect(() => {
    if (!form.producto_id) return;
    const exists = productosDisponiblesParaOrigen.some((prod) => String(prod.id) === String(form.producto_id));
    if (!exists) {
      setForm((prev) => ({
        ...prev,
        producto_id: "",
  ...
  }, [form.producto_id, productosDisponiblesParaOrigen]);
  ...
  const availableOriginZones = useMemo(() => {
    ...
    return ZONAS.filter((zona) => {
      if (form.tamano_origen) {
        const key = `${form.producto_id}__${String(zona).toLowerCase()}__${form.tamano_origen}`;
        return Number(stockByProductZoneSize.get(key) || 0) > 0;
      }

      return TAMANOS.some((tamano) => {
        const key = `${form.producto_id}__${String(zona).toLowerCase()}__${tamano}`;
        return Number(stockByProductZoneSize.get(key) || 0) > 0;
      });
    });
  ...
  const availableOriginSizes = useMemo(() => {
    ...
    return TAMANOS.filter((tamano) => {
      const key = `${form.producto_id}__${String(form.zona_origen).toLowerCase()}__${tamano}`;
      return Number(stockByProductZoneSize.get(key) || 0) > 0;
    });
                    ...
                    {pedidoLineas.map((linea) => {
                      ...
                      return (
                        ...
                        >
                          <div>
                            <div style={{ fontWeight: 900, color: "#0f172a" }}>
                              {linea.producto_nombre_cientifico || linea.nombre_cientifico || linea.producto_nombre || `Producto #${linea.producto_id}`}
                            </div>
                            <div style={{ marginTop: 4, color: "#64748b", fontWeight: 700 }}>
            ...
            >
              ...
              <div>
                ...
                <select
                  ...
                  style={inputStyle()}
                  disabled={form.origen_tipo === "Vivero" ? productosDisponiblesParaOrigen.length === 0 : false}
                >
                  <option value="">
                    {form.origen_tipo === "Vivero"
                      ? productosDisponiblesParaOrigen.length === 0
                        ? "No hay productos con stock en vivero"
                        : "Seleccionar producto"
                      : "Seleccionar producto"}
                  </option>
                  {productosDisponiblesParaOrigen.map((p) => (
            ...
            >
              ...
              >
                ...
                <div style={{ color: "rgba(255,255,255,0.8)", fontWeight: 700, display: "grid", gap: 8 }}>
                  ...
                  <div>• Osana, Otro, Empresa, Organismo oficial, Colegio y Palmetum solo pueden entrar al vivero.</div>
...
export default function Movimientos() {
  ...
  const movimientosFiltrados = useMemo(() => {
    return movimientos.filter((m) => {
      ...
      const productoMatch =
        !productoTxt ||
        `${m?.producto_nombre_cientifico || ""} ${m?.nombre_cientifico || ""} ${m?.producto_id || ""}`
          .toLowerCase()
