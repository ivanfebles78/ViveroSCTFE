  ...
  try {
    for (const alias of aliases) {
      try {
        ...
        // Guardamos también la respuesta vacía por si ninguna otra variante devuelve datos
        if (!lastError) {
          lastError = { empty: true, data };
        }
      } catch (e) {
        lastError = e;
      }
    ...
    // Si ninguna variante devolvió datos, mostramos la mejor respuesta disponible
    if (lastError?.empty) {
      setZonaData({
        ...(lastError.data || {}),
        _resolvedZone: aliases[0],
      });
      setZonaError("");
    } else {
      setZonaData(null);
      ...
      >
        ...
        <div style={{ padding: 20, overflow: "auto" }}>
          <div style={{ fontSize: 22, fontWeight: 900, color: "#0f172a" }}>
            {selectedZone ? `Zona ${selectedZone}` : "Selecciona una zona"}
          </div>
            ...
            <>
              ...
              <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 10 }}>
                {(zonaData?.items || []).length === 0 ? (
                  <div style={{ color: "#64748b", fontWeight: 700 }}>No hay productos en esta zona.</div>
                ) : (
