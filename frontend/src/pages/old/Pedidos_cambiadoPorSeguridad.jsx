...
function buildStockByProductSizeFromMovimientos(movimientos) {
  const map = new Map();
...
function buildStockByProductSize(productos, movimientos) {
  const fromBackend = new Map();

  for (const p of safeArray(productos)) {
    const stockBySize = p?.stock_by_size || p?.stockBySize || null;
    if (stockBySize && typeof stockBySize === "object") {
      TAMANOS.forEach((tam) => {
        const val = Number(stockBySize[tam] || 0);
        fromBackend.set(lineKey(p.id, tam), Math.max(0, val));
      });
    }
  }

  if (fromBackend.size > 0) return fromBackend;
  return buildStockByProductSizeFromMovimientos(movimientos);
}
...
function getProductDisplayName(p) {
  return p?.nombre_cientifico || p?.nombre || p?.nombre_natural || `Producto #${p?.id}`;
}

function getPedidoItemsView(pedido, editingId, editQty) {
  if (editingId !== pedido.id) return safeArray(pedido.items);

  return Object.entries(editQty).map(([key, cantidad]) => {
...
function buildMovimientoHoverText(item, pedidoId, movimientos) {
  const propios = Array.isArray(item?.movimientos_servicio) ? item.movimientos_servicio : [];
  const fallback = Array.isArray(movimientos)
    ? movimientos.filter((m) => {
        const samePedido = String(m?.pedido_id || "") === String(pedidoId || "");
        const sameProducto = String(m?.producto_id || "") === String(item?.producto_id || "");
        const sameTamano = String(m?.tamano_origen || m?.tamano_destino || "") === String(item?.tamano || "");
        return samePedido && sameProducto && sameTamano;
      })
    : [];

  const movs = propios.length ? propios : fallback;
  if (!movs.length) return "Sin movimientos de servicio todavía.";

  return movs
    .map((m) => {
      const destino = [m?.distrito_destino, m?.barrio_destino, m?.direccion_destino]
        .filter(Boolean)
        .join(" · ");

      return [
        `Movimiento #${m?.movimiento_id || m?.id || "—"}`,
        `Fecha: ${m?.fecha_movimiento ? new Date(m.fecha_movimiento).toLocaleString("es-ES") : "—"}`,
        `Cantidad: ${m?.cantidad || 0}`,
        `Origen: ${m?.origen_tipo || "—"}${m?.zona_origen ? ` · Zona ${m.zona_origen}` : ""}${m?.tamano_origen ? ` · ${m.tamano_origen}` : ""}`,
        `Destino: ${m?.destino_tipo || "—"}${destino ? ` · ${destino}` : ""}`,
...
function PedidoDetalleCell({
  ...
  const items = getPedidoItemsView(pedido, editingId, editQty);
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {items.length ? (
          visibleItems.map((it, idx) => {
            ...
            const hoverText = buildMovimientoHoverText(it, pedido.id, movimientos);
            const hasService =
              (Array.isArray(it?.movimientos_servicio) && it.movimientos_servicio.length > 0) ||
              (Array.isArray(movimientos) &&
                movimientos.some(
                  (m) =>
                    String(m?.pedido_id || "") === String(pedido.id) &&
                    String(m?.producto_id || "") === String(it?.producto_id || "") &&
                    String(m?.tamano_origen || m?.tamano_destino || "") === String(it?.tamano || "")
                ));

            return (
              <div
                key={`${pedido.id}-detalle-${idx}-${pid}`}
                title={hoverText}
                style={{
                  color: "#0f172a",
                  fontWeight: 800,
                  ...
                  padding: "8px 10px",
                  borderRadius: 10,
                  background: hasService ? "rgba(16,185,129,0.08)" : "transparent",
                  border: hasService ? "1px solid rgba(16,185,129,0.18)" : "1px solid transparent",
                  cursor: hasService ? "help" : "default",
                }}
              >
                {nombre}
                {typeof it?.cantidad_servida !== "undefined"
                  ? ` · ${Number(it.cantidad_servida || 0)}/${Number(it.cantidad || 0)}`
                  : ""}
              </div>
            );
          })
            ...
            {productosDisponiblesParaEdicion.length === 0 ? (
              ...
              productosDisponiblesParaEdicion.flatMap((prod) =>
                TAMANOS.map((tam) => {
                  ...
                  const disponible = Number(stockByProductSize.get(key) || 0);
                  if (editQty[key] != null || disponible <= 0) return null;

                  return (
                    <div
                      key={`${pedido.id}-add-${key}`}
                      style={{
                        display: "grid",
                        gridTemplateColumns: "1fr 88px 88px auto",
                        gap: 10,
                        alignItems: "center",
                        padding: "10px 12px",
...
function PedidoTamanoCell({ pedido, editingId, editQty, expanded }) {
  const items = getPedidoItemsView(pedido, editingId, editQty);
  const visibleItems = expanded ? items : items.slice(0, 3);
...
function PedidoCantidadCell({ pedido, editingId, editQty, expanded, stockByProductSize, setEditQty }) {
  const items = getPedidoItemsView(pedido, editingId, editQty);
  const visibleItems = expanded ? items : items.slice(0, 3);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {visibleItems.map((it, idx) => {
        const key = lineKey(it.producto_id, it.tamano);
        const max = Number(stockByProductSize.get(key) || 0);

        return (
          <input
            key={`${pedido.id}-edit-qty-${idx}-${key}`}
            type="number"
            min={0}
            max={max}
            ...
            onChange={(e) =>
              setEditQty((prev) => ({
                ...prev,
                [key]: clampNumber(e.target.value, 0, max),
              }))
            }
            style={{
              width: 82,
  ...
  const productosDisponibles = useMemo(() => {
    ...
    return safeArray(productos)
      .filter((p) => {
        ...
        const nombre = String(getProductDisplayName(p)).toLowerCase();
      ...
      .sort((a, b) => String(getProductDisplayName(a)).localeCompare(String(getProductDisplayName(b))));
    ...
    return Object.entries(cart)
      .map(([key, cantidad]) => {
        ...
        return {
          ...
          tamano: parsed.tamano,
          cantidad: Number(cantidad),
          nombre: getProductDisplayName(prod),
          disponible,
        };
      })
      ...
      >
        ...
        >
          <div style={{ fontSize: 24, fontWeight: 900, color: "#0f172a" }}>
            {selectedProduct ? getProductDisplayName(selectedProduct) : "Selecciona un producto"}
          </div>
...
export default function Pedidos() {
  ...
  const stockByProductSize = useMemo(
    () => buildStockByProductSize(productos, movimientos),
  ...
  const mapProdName = useMemo(() => {
    ...
    for (const p of productos) {
      m.set(p.id, getProductDisplayName(p));
    }
    return m;
  }, [productos]);
  ...
  const editValidation = useMemo(() => {
    if (!editingId) return { valid: true, reason: "" };

    const entries = Object.entries(editQty);
    if (!entries.length) {
      return { valid: false, reason: "El pedido debe tener al menos 1 línea." };
    }

    for (const [key, cantidad] of entries) {
      const max = Number(stockByProductSize.get(key) || 0);
      const qty = Number(cantidad || 0);

      if (!qty || qty <= 0) {
        return { valid: false, reason: "Todas las cantidades deben ser mayores que 0." };
      }
      ...
      if (qty > max) {
        ...
        return {
          valid: false,
          reason: `Stock insuficiente para ${nombre} en tamaño ${parsed.tamano}. Disponible=${max}, solicitado=${qty}`,
        };
      }
    }

    return { valid: true, reason: "" };
  }, [editingId, editQty, stockByProductSize, mapProdName]);

  const onGuardarEdicion = async (pedidoId) => {
    if (!editValidation.valid) {
      showTimedMessage(editValidation.reason, "error");
      return;
    }

    try {
      const pedidoOriginal = pedidos.find((x) => x.id === pedidoId);
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      >
        <div>
          <h1 style={{ fontSize: 46, margin: 0, fontWeight: 950, color: "#0f172a", letterSpacing: -1.2 }}>
            Pedidos
          </h1>
          <div style={{ marginTop: 8, color: "#64748b", fontWeight: 700 }}>
            Gestión visual y trazable de reservas del vivero.
          </div>
        </div>

        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <button onClick={() => setModalOpen(true)} style={primaryBtn(false)}>
            Nuevo pedido
          </button>
      ...
      >
        ...
        >
          ...
          <input
            placeholder="Buscar texto en detalle, destino, estado, ID..."
            value={textoFiltro}
        ...
        >
          Mostrando <b>{pedidosFiltrados.length}</b> de <b>{pedidos.length}</b> pedidos
        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                          ...
                          editingId !== p.id ? (
                            ...
                            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                              <button
                                ...
                                style={primaryBtn(!editValidation.valid)}
                              >
                                Guardar
                              </button>
                        ...
                        {editingId === p.id && !editValidation.valid ? (
                          <div style={{ marginTop: 8, fontSize: 12, color: "#991b1b", fontWeight: 800 }}>
