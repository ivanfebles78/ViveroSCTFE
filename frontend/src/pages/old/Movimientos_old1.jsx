...
function isExternalDestination(value) {
  return String(value || "").trim().toLowerCase() === "externo";
}
...
function PrestamoSelectorModal({ open, prestamos, onClose, onSelect }) {
  const [texto, setTexto] = useState("");
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(2,6,23,0.45)", backdropFilter: "blur(3px)", zIndex: 1300, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ width: "min(980px, 95vw)", maxHeight: "88vh", overflow: "hidden", background: "white", borderRadius: 24, boxShadow: "0 30px 80px rgba(2,6,23,0.35)", border: "1px solid rgba(15,23,42,0.10)", display: "grid", gridTemplateRows: "auto auto 1fr" }}>
        <div style={{ padding: "20px 22px 10px", display: "flex", alignItems: "start", justifyContent: "space-between", gap: 16 }}>
          <div>
            <div style={{ fontSize: 26, fontWeight: 900, color: "#0f172a" }}>Préstamos activos</div>
            <div style={{ marginTop: 6, color: "#64748b", fontWeight: 700 }}>Selecciona un préstamo activo para preparar su devolución.</div>
          </div>
          <button onClick={onClose} style={closeButtonStyle()}>Cerrar</button>
        </div>

        <div style={{ padding: "0 22px 14px" }}>
          <input value={texto} onChange={(e) => setTexto(e.target.value)} placeholder="Buscar por ID, producto, origen, destino..." style={inputStyle()} />
        </div>

        <div style={{ padding: "0 22px 22px", overflow: "auto" }}>
          {prestamosFiltrados.length === 0 ? (
            <div style={{ border: "1px solid rgba(15,23,42,0.08)", borderRadius: 16, padding: 18, color: "#64748b", fontWeight: 800 }}>
              No hay préstamos activos que coincidan con la búsqueda.
            </div>
          ) : (
            <div style={{ display: "grid", gap: 14 }}>
              {prestamosFiltrados.map((p) => (
                <div key={p.id} style={{ border: "1px solid rgba(15,23,42,0.08)", borderRadius: 18, padding: 16, background: "#fbfdff" }}>
                  <div style={{ display: "flex", gap: 12, alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" }}>
                    <div>
                      <div style={{ fontSize: 18, fontWeight: 900, color: "#0f172a" }}>Préstamo #{p.id}</div>
                      <div style={{ marginTop: 4, color: "#64748b", fontWeight: 700 }}>{fmtFechaES(p.fecha_movimiento)} · {p.origen_tipo || "—"} → {p.destino_tipo || "—"}</div>
                    </div>
                    <button
                      onClick={() => onSelect(p)}
                      style={{ padding: "10px 14px", borderRadius: 14, border: "1px solid rgba(234,179,8,0.35)", background: "rgba(234,179,8,0.12)", color: "#92400e", fontWeight: 900, cursor: "pointer" }}
                    >
                      Usar préstamo
                    </button>
                  </div>
                  ...
                  <div style={{ marginTop: 10, color: "#475569", fontWeight: 700 }}>
                    Producto: {p.producto_nombre || p.producto_nombre_natural || p.producto_nombre_cientifico || `Producto #${p.producto_id}`}
                  </div>
                  <div style={{ marginTop: 6, color: "#475569", fontWeight: 700 }}>
                    Cantidad: {p.cantidad || 0} · Tamaño: {p.tamano_origen || p.tamano_destino || "—"} · Zona original: {p.zona_origen || "—"}
                  </div>
                  <div style={{ marginTop: 6, color: "#475569", fontWeight: 700 }}>
                    Destino externo: {[p.distrito_destino, p.barrio_destino, p.direccion_destino].filter(Boolean).join(" · ") || p.destino_tipo || "—"}
                  </div>
                </div>
              ))}
            </div>
...
function MovimientoModal({
  ...
  const usarPrestamoActivo = (prestamo) => {
    setForm((prev) => ({
      ...prev,
      pedido_id: "",
      pedido_item_id: "",
      prestamo_referencia_id: String(prestamo.id || ""),
      producto_id: String(prestamo.producto_id || ""),
      cantidad: String(prestamo.cantidad || ""),
      origen_tipo: prestamo.destino_tipo || "Otro",
      destino_tipo: "Vivero",
      zona_origen: "",
      tamano_origen: "",
      zona_destino: prestamo.zona_origen || prev.zona_destino || "",
      tamano_destino: prestamo.tamano_origen || prev.tamano_destino || "",
      distrito_destino: "",
      barrio_destino: "",
      direccion_destino: "",
      cp_destino: "",
      nota: prev.nota || `Devolución del préstamo #${prestamo.id}`,
      observaciones: prev.observaciones || `Devolución del préstamo #${prestamo.id}`,
      prestamo: false,
    }));
    setSelectedPedidoLineKey("");
    setErrors([]);
  ...
  const submit = async () => {
    ...
    await onSubmit({
      ...
      prestamo_referencia_id: form.prestamo_referencia_id ? Number(form.prestamo_referencia_id) : null,
      ...
      observaciones: form.observaciones || form.nota || null,
      nota: form.observaciones || form.nota || null,
      es_prestamo: false,
      es_devolucion: !!form.prestamo_referencia_id,
    });
  };
            ...
            >
              ...
              >
                ...
                <div style={{ color: "rgba(255,255,255,0.8)", fontWeight: 700, display: "grid", gap: 8 }}>
                  ...
                  <div>• También puedes preparar una devolución desde un préstamo activo.</div>
  ...
  const load = async () => {
    ...
    try {
      ...
      try {
        ...
        setPrestamosActivos(safeArray(prestamos));
      } catch (prestamoError) {
        console.warn("No se pudieron cargar los préstamos activos:", prestamoError);
        setPrestamosActivos([]);
      }
    } catch (e) {
      showTimedMessage(
                ...
                {movimientosFiltrados.map((m) => (
                  ...
                  >
                    ...
                    <td style={{ ...tdStyle(), borderTop: "1px solid rgba(15,23,42,0.10)", borderBottom: "1px solid rgba(15,23,42,0.10)" }}>
                      <span style={badgeTipo(m.tipo_movimiento || getMovimientoTipo(m))}>{m.tipo_movimiento || getMovimientoTipo(m)}</span>
                    </td>
