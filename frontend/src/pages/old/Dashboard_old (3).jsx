...
function buildCaducidadItems(productos) {
  ...
  (Array.isArray(productos) ? productos : []).forEach((producto) => {
    ...
    alertas.forEach((alerta, idx) => {
      pushItem({
        id: alerta?.id || `alerta-${producto?.id || idx}`,
    ...
    lotes.forEach((lote, idx) => {
      pushItem({
        id: lote?.id || lote?.uuid_lote || lote?.uuid || `lote-${producto?.id || idx}`,
    ...
    if (producto?.fecha_caducidad) {
      pushItem({
        id: `producto-${producto?.id || "sin-id"}`,
        zona: producto?.zona,
  ...
  return (
    ...
    >
      ...
      <div style={{ ...cardStyle, gridColumn: "1 / span 2" }}>
        ...
        {caducidadChart.total === 0 ? (
          <div style={{ color: "#64748b", fontWeight: 700 }}>
            No hay datos de caducidad disponibles para mostrar.
          </div>
        ) : (
          <div
            style={{
          ...
          >
            <div style={{ display: "flex", justifyContent: "center" }}>
              <DonutChart
                ...
                centerTop={String(caducidadChart.total)}
                centerBottom="registros"
              />
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              >
                ...
                >
                  <div style={{ color: "#64748b", fontWeight: 800, fontSize: 12 }}>Bien de fecha</div>
                  <div style={{ color: "#0f172a", fontWeight: 900, fontSize: 24, marginTop: 4 }}>
                    {caducidadChart.bien}
                  </div>
                </div>
                ...
                >
                  <div style={{ color: "#64748b", fontWeight: 800, fontSize: 12 }}>Próximos a caducar</div>
                  <div style={{ color: "#0f172a", fontWeight: 900, fontSize: 24, marginTop: 4 }}>
                    {caducidadChart.proximo}
                  </div>
                </div>
                ...
                >
                  <div style={{ color: "#64748b", fontWeight: 800, fontSize: 12 }}>Caducados</div>
                  <div style={{ color: "#0f172a", fontWeight: 900, fontSize: 24, marginTop: 4 }}>
                    {caducidadChart.caducado}
                  </div>
                </div>
              </div>
            </div>
...
export default function Dashboard() {
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      totalCaducidad: caducidadItems.length,
    };
  }, [productos, pedidos, caducidadItems]);
